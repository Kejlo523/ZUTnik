...
/**
 * Handles the OAuth 1.0a callback deep link: zutnik://oauth/callback
 *
 * Flow:
 *   Browser  →  zutnik://oauth/callback?oauth_token=X&oauth_verifier=Y
 *   ↓
...
public class UsosOAuthCallbackActivity extends PhoneAwareActivity {
    ...
    static final String PREFS_NAME              = "zutnik_prefs";
    ...
    private void doTokenExchange(
        ...
        executor.execute(() -> {
            try {
                ...
                // ── Step 3: Save session ────────────────────────────────────────────
                ZutnikSession session = ZutnikSession.getInstance(UsosOAuthCallbackActivity.this);
